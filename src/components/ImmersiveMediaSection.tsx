...
export const ImmersiveMediaSection: React.FC<ImmersiveMediaSectionProps> = ({
  projects,
  onSelectProject,
  onHoverProject
}) => {
  ...
  const filtered = projects.filter(p => p.category === 'IMMERSIVE MEDIA' && p.published);
    ...
    >
      ...
      <motion.div variants={itemVariants} className="mb-12 sm:mb-16 space-y-4">
        ...
        <p className="font-mono text-xs text-white/50 uppercase tracking-[0.2em] max-w-2xl leading-relaxed">
          Virtual Production &bull; Real-Time Environments &bull; VR / Spatial Computing &bull; Interactive Installations
        </p>
      </motion.div>
      ...
      {filtered.length === 0 ? (
        <motion.div variants={itemVariants} className="py-24 text-center border border-dashed border-white/10 p-8">
          <p className="font-mono text-xs text-white/40 uppercase tracking-[0.2em]">
            No published Immersive Media projects yet. Add projects via the Admin Dashboard.
          </p>
        </motion.div>
      ) : (
